...
Function.prototype.decorate = ({before, after, is-passing-arguments, is-before-async})-> # async 只有对before有意义
  let b = before, a = after, self = @ # 注意，必须用闭包保证回调时before、this、after的正确性！
    if is-before-async
      if is-passing-arguments # 将before执行完结果，作为参数给self，self执行完的结果，作为参数给after
        -> 
          if typeof &[&.length - 1] is 'function' # 最后一个是回调中要执行的方法
            args = Array.prototype.slice.call &, 0, &.length - 1
            callback = &[&.length - 1]
          else
            args = &
        ...
        -> 
          if typeof &[&.length - 1] is 'function' # 最后一个是回调中要执行的方法
            ...
            args = &
          Array.prototype.push.call args, !-> callback? ... ; self ... ; a? ...
          b.apply null, args
    else
      if is-passing-arguments # 将before执行完结果，作为参数给self，self执行完的结果，作为参数给after
        -> 
...
visit-leaf = (obj, visitor, path='')-> 
  for own key, value of obj
    path = move-path-down path, key
    visit visitor, value, path
    path = move-path-up path

visit = (visitor, value, path)->
  if typeof value is 'object'
    if Array.is-array value then visit-array value, visitor, path else visit-leaf value, visitor, path
  else
    visitor value, path


move-path-down = (path, key)-> if path is '' then key else "#{path}.#{key}"

move-path-up = (path)-> (path.split '.')[0 to -2].join '.'

visit-array = (array, visitor, path)->
  for element, index in array
    old-path = path ; path += "[#{index}]"
...
# test-decorate = ->
#   a = (p)-> console.log "a: ", p + 1
#   b = (p, fn)->
#     console.log "b: ", p++
#     fn p
#   a.decorate {before: b, is-before-async: true}

# root = module?.exports ? @
# root.test-decorate = test-decorate
